...
- `--pw` / `--no-pw` - Enable/disable Playwright browser automation
...
| /user:playwright-test | UI verification w/ Playwright | --url --flows --snapshots --console --pause |
...
Quality Workflows:
  ...
  Code Review (UI): analyze --code --think → playwright-test --url <url> --snapshots --console
...
Power User Patterns:
  ...
  UI Development: build --react --magic --pw --watch
  UI Verification: playwright-test --url <url> --flows --snapshots --console
